...
FILENAME = 'LeNet'+str(BATCH_SIZE)


def leNet_model(shape, activation = ACTIVATION):
	"""Create a Convolutional network model with LeNet architecture
	...
	x = Conv2D(6,kernel_size=(5, 5), strides= (1,1),padding='same',activation=activation,kernel_regularizer=l2(REG))(inp)
	x = AveragePooling2D(pool_size=(2, 2), strides= (2,2))(x)
	x = Conv2D(16,kernel_size=(5, 5), strides= (1,1),activation=activation,kernel_regularizer=l2(REG))(x)
	x = AveragePooling2D(pool_size=(2, 2), strides= (2,2))(x)
	x = Conv2D(120,kernel_size=(5, 5), strides= (1,1),activation=activation,kernel_regularizer=l2(REG))(x)
	x = Flatten()(x)
	x = Dense(84,activation=activation,kernel_regularizer=l2(REG))(x)
...
def preprocessing(data, X_dim = 4):
...
if __name__ == '__main__':
	...
	# Store data to csv for future use
